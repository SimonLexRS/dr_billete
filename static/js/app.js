...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function showResults(panel, data, container) {
        ...
        const xIcon = `<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.744l7.73-8.835L1.254 2.25H8.08l4.253 5.622zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>`;
        const copyIcon = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>`;
        ...
        html += `
            <div class="share-section">
                ...
                <div class="share-buttons">
                    <a class="share-btn share-wa" href="https://wa.me/?text=${encodeURIComponent(shareText + ' ' + shareUrl)}" target="_blank" rel="noopener">${waIcon} WhatsApp</a>
                    <a class="share-btn share-fb" href="https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl)}&quote=${encodeURIComponent(shareText)}" target="_blank" rel="noopener">${fbIcon} Facebook</a>
                    <a class="share-btn share-x" href="https://twitter.com/intent/tweet?text=${encodeURIComponent(shareText)}&url=${encodeURIComponent(shareUrl)}" target="_blank" rel="noopener">${xIcon} X</a>
                    <button class="share-btn share-copy" onclick="navigator.clipboard.writeText('${safeShareText} ${shareUrl}').then(()=>{this.querySelector('span').textContent='Copiado!';setTimeout(()=>{this.querySelector('span').textContent='Copiar'},2000)})">${copyIcon}<span>Copiar</span></button>
                </div>
            </div>
            <button class="scan-another-btn" onclick="resetScan('${container ? container.id : ''}')">
                &#8592; Escanear otro billete
